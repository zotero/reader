...
test('realigns local text-layer insertions and omissions', () => {
	assert.deepEqual(
		alignTextUnits(getNormalizedUnits('ABXCD'), getNormalizedUnits('ABCD')),
		[0, 1, 2, 2, 3, 4]
	);
	assert.deepEqual(
		alignTextUnits(getNormalizedUnits('ABCD'), getNormalizedUnits('ABXCD')),
		[0, 1, 3, 4, 5]
	);
});
...
test('aligns RTL text without changing logical order', () => {
	let domUnits = getNormalizedUnits('שלום Xעולם');
...
test('aligns CJK text with layout whitespace omitted', () => {
	let readerUnits = getNormalizedUnits('你好世界');
	assert.deepEqual(
		alignTextUnits(getNormalizedUnits('你好 世界'), readerUnits),
