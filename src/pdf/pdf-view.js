...
import { getPageScrollTarget, getScrollTarget } from './scroll-target.mjs';
import {
...
class PDFView {
	...
	navigateToPosition(position, options = {}) {
		...
		let page = this._options.platform === 'android' && !options.ifNeeded && !options.inline
			&& (options.block ?? 'center') === 'center'
			? pdfViewer.getPageView(position.pageIndex)
			: null;
		if (page?.pdfPage) {
			let { div } = page;
			let horizontal = pdfViewer.scrollMode === HORIZONTAL_SCROLL_MODE;
			let bounds = div.getBoundingClientRect();
			let pageLeft = bounds.left + scrollLeft + div.clientLeft;
			let pageTop = bounds.top + scrollTop + div.clientTop;
			let pageTarget = getPageScrollTarget({
				rect,
				pageRect: [pageLeft, pageTop, pageLeft + div.clientWidth, pageTop + div.clientHeight],
				scrollLeft,
				clientWidth,
				clientHeight,
				horizontal,
			});
			left = pageTarget.left;
			// Don't hide a cross-page annotation's continuation below the viewport.
			if (!position.nextPageRects || horizontal) {
				top = pageTarget.top;
			}
		}
